...
Multi-Plane Autoregressive Diffusion (MPAD) compresses MRI volumes with a 3D KL-regularized autoencoder and synthesizes target contrasts with a 2D diffusion model that runs autoregressively across orthogonal latent planes. Training and inference are split into two stages that share one environment.
